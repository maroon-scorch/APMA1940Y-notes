...
Let's recall the Continuous Wavelet Transform:
\[(W_\psi f)(\alpha, a) = \frac{1}{\sqrt{a}} \int_{-\infty}^{\infty} f(t) \psi(\frac{t = \alpjha}{a}) dt\]
...
The Discrete (or Fast) Wavelet Transform does this in efficient time with $O(1)$ space complexity, meaning there's no new memory allocations. It also turns out this is a bijective transforms so you could start with $\{d_k^{(n)}\}$ and getting back $\{c_k^{(n)}\}$ just as fast. We will talk more about this next lecture.
\end{document}
