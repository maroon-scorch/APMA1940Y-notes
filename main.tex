...
Indeed, let $f \in L^2(\Rbb^2)$ be given and fix $y \in \Rbb$, then
\[f(\bullet, y): \Rbb \to \Rbb \text{ is a univariate function in $L^2(\Rbb)$}\]
Hence, we can write
\[f(x, y) = \sum_{m \in \Zbb} \sum_{k \in \Zbb} d_k^{(m)}(y) \psi(2^m x - k)\]
where the coefficients $d_k^{(m)}(y)$ depend on the indices and $y$.\\\\

Then for fixed $m, k \in \Zbb$, we decompose
\begin{align*}
    d_k^{(m)}(y) = \sum_{n \in \Zbb} \sum_{\ell \in \Zbb} d_{k, \ell}^{(m, n)} \psi(2^n y - \ell)
\end{align*}
for suitable constants $\{d_{k, \ell}^{(m, n)}$. Combining this gives
\[f(x, y) = \sum_{(m, n) \in \Zbb^2} \sum_{(k, \ell) \in \Zbb^2} d_{k, \ell}^{(m, n)} \psi(2^m x - k) \psi(2^n y - \ell) \quad (\dagger)\]
which is the bivariate wavelet decomposition of $f \in L^2(\Rbb^2)$, which is really the tensor product of the two univariate decompositions.\\

In particular, if $\{\psi(\bullet - k)\}$ are orthonormal, then $(\dagger)$ is still an orthogonal decomposition, hence
\[||f||_{L^2(\Rbb^2)} = \sum_{(m, n) \in \Zbb^2} \sum_{(k, \ell) \in \Zbb^2} 2^{m + n} |d_{k, \ell}^{(m, n)}|^2\]

Essentially everything in $\Rbb^1$ can be generalized to $\Rbb^d$ using tensor products.
\begin{question}
    How to compute the $d_{k, \ell}^{(m, n)}$ efficiently?
\end{question}

Let's recall in the $1D$ case of Haar Wavelets, 
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-1.png}\]
For the Fast Wavelet Transform, we essentially wrote the new coefficients $c_k^{(n)}$ and $d_k^{(n)}$ as an average and a difference.\\

To generalize this to $2D$, we now have a grid/matrix of coefficients:
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-2.png}\]
One possible option is that - we can perform the 1D discrete wavelet transform on each row of the matrix to completion and apply the same 1D-DWT to each resulting column that's not changed. This is possibly the easiest way to code it recursively.\\

However, we end up with the coefficients $\{d_{k,\ell}^{(m, n)} \}$, but it may not always be the best way to understand or interpret the 2D decomposition.\\

In the 1D case of Haar Wavelets, we relied on these recursive averaging and differencing locally around the coefficients. Here's an \ul{alternative option} - motivated by the geometric interpretation of the 1D DWT, we want to try to work with small ``cells" of coefficients instead.\\

 Let's consider the cells composing of $2 \times 2$ coefficients (the dimension depends on what wavelet you are using, but for Haar it is this) - 
 \[\begin{bmatrix} c_{00} & c_{01}\\ c_{10} & c_{11} \end{bmatrix}\]
 Note that the top coefficients are fixed to be $1$ for simplicity.

\begin{question}
    How to interpret this cell above? What do we mean by the $2 \times 2$ array?
\end{question}

We observe that
\begin{itemize}
    \item We can write
    \[f^{(1)}(x, y) = \sum_{m , n} c_{m, n}^(\ell) \varphi(2x - m) \varphi(2y - n)\]
    \item $c_{00}$ is the coefficient of $\varphi(2x) \varphi(2y)$
    \item $c_{01}$ is the coeffieint of $\varphi(2x) \varphi(2y - 1)$
\end{itemize}

\mattie{$\varphi$ is the step function, $\psi$ is the Haar Wavelet} Geometrically, what does $\varphi(2x) \varphi(2y)$ look like?
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-3.png}\]
Note that the $x$ and $y$ axis are flipped here (look at the matrix above). We want to decompose this into a Wavelet basis. Now in $1D$ we had,
\[V_1 = V_0 \oplus W_0\]
Hence, 
\[V_1 \otimes V_1 = (V_0 \oplus W_0) \otimes (V_0 \oplus W_0) = V_0 \otimes V_0 \oplus V_0 \otimes W_0 \oplus W_0 \otimes V_0 \oplus W_0 \otimes W_0\]

What does each $(V, W)_0 \otimes (V, W)_0$ look like? Well, we know $V_0$ is given by $\varphi$ and $W_0$ is given by $\psi$, so we see they are:
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-4.png}\]

\begin{question}
    Is it obvious that $\varphi(x) \otimes \varphi(y)$, $\varphi(x) \otimes \psi(y)$, $\psi(x) \otimes \varphi(y)$, and $\psi(x) \otimes \psi(y)$ form an orthogonal basis for $V_1 \otimes V_1$?\\

    Yes, this is a tensor product of vector spaces, and hence we have an alternative orthogonal basis for $V_1 \otimes V_1$.
\end{question}

Why don't we write the coefficients $\begin{bmatrix} c_{00} & c_{01}\\ c_{10} & c_{11} \end{bmatrix}$ in this basis? The resulting matrix is still $2 \times 2$ matrix and given by
\[\begin{bmatrix} c_{00}^{new} & d_{01}^{new}\\ d_{10}^{new} & d_{11}^{new} \end{bmatrix}\]

Well we have the equation
\[\begin{bmatrix} c_{00} & c_{01}\\ c_{10} & c_{11} \end{bmatrix} = c_{00}^{new} \begin{bmatrix} 1 & 1\\ 1 & 1\end{bmatrix} + d_{01}^{new} \begin{bmatrix} 1 & -1\\ 1 & -1\end{bmatrix} + d_{10}^{new} \begin{bmatrix} 1 & 1\\ -1 & - 1\end{bmatrix} + d_{11}^{new} \begin{bmatrix}
    1 & -1\\ -1 & 1
\end{bmatrix}  \]
We can extract the coefficients by taking the \textbf{kronecker product} on both sides and hence
\begin{itemize}
    \item $c_{00}^{new} = \frac{1}{4} \left( c_{00} + c_{01} + c_{10} + c_{11} \right)$
    \item $d_{01}^{new} = \frac{1}{4} \left( c_{00} - c_{01} + c_{10} - c_{11} \right)$
    \item $d_{10}^{new} = \frac{1}{4} \left( c_{00} + c_{01} - c_{10} - c_{11} \right)$
    \item $d_{11}^{new} = \frac{1}{4} \left( c_{00} - c_{01} - c_{10} + c_{11}\right)$
\end{itemize}

Summarizing, the cell operation gives us
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-5.png}\]

This whole decomposition also applies this ``atomic" operation to $2 \times 2$ sub-blocks recursively.\\

Thinking about this in terms of frequencies (low frequency is averaging, high freqeuncy is differencing):
\[\includegraphics[width=0.5\textwidth]{Figures/lec26-6.png}\]


\newpage
\section{Lecture 27 - 4/5/2023}

\subsection{Multi-dimensional Wavelets (continued)}

Last lecture, we considered $2 \times 2$ cells (in the case of Haar), the Discrete Wavelet Transform of that allowed us to consider:
\[\begin{bmatrix}
...
\end{bmatrix}\]
\begin{enumerate}
    \item The first matrix is thought of having low frequency in both $x$ and $y$ component (LL)
    \item The second matrix is thought of having low frequency in $x$ and high frequency in $y$ (LH)
    \item The third matrix is thought of having high frequency in $x$ and low frequency in $y$ (HL)
    \item The fouth matrix is thought of having high frequency in both $x$ and $y$ component (HH)
\end{enumerate}
In principle, we can do the DWT recursively as follows:
\[\includegraphics[width=0.7\textwidth]{Figures/lec27-1.png}\]

The runtime of this algorithm for a given $N$ pieces is
\[16N + 16(\frac{N}{4}) + 16(\frac{N}{4^2}) + ... \to CN\]
where $C$ is some constant. So the run time is again linear with respective to the input.\\
...
One of the earliest applications of wavelets was to help the FBI store fingerprints efficiently. This was originally attempted with the fast Fourier Transform. The problem with the Fast Fourier Transform is that it blurs the edges of the finger prints.\\

The following is an example of using DWT on the finger print:
\[\includegraphics[width=0.4\textwidth]{Figures/lec27-2.png}\]
We observe that the top right picture contains the horizontal frequency parts and the bottom left picture contains the vertical frequency parts.\\

What we can do is store the result of the DWT after applying the threshold method or the alternative method. This means we have less data to store, but we can recover the original image fairly well by applying the Inverse Wavelet Transform.\\

Raising the percentage of what to remove may decrease the accuracy of the recovered image, but this can be remedied by doing more steps in the DWT (the figure before only did DWT once) -
\[\includegraphics[width=0.4\textwidth]{Figures/lec27-3.png}\]
Zeroing out the $90\%$ and applying IWT gives us
\[\includegraphics[width=0.4\textwidth]{Figures/lec27-4.png}\]
\end{example}

\begin{question}
    We do obtain a sparse image at the end, but how can we efficiently store this image efficiently?
\end{question}

There's a general procedure called the Huffman Encoding to do this. It turns out is a standard application of \textbf{coding theory}, which is also founded by Shannon.

\newpage
\section{Lecture 28 - 4/7/2023}

\subsection{Solution to Final Part of Hmework 6}
...
\end{align*}
The error term $\sum_{k \text{ odd} (f(t) - c_k)^2$ is just the usual MRA given by the step function and hence goes to $0$ as $n \to \infty$. The same estimate holds similarly for even term. Hence, we conclude that
\[\lim_{n \to \infty} ||f - f_n||_2 = 0\]

\subsection{Orthonormal Wavelets}
So far, we have only constructed one example of wavelet that gives us an MRA - the Haar Wavelet, but we would like some other examples too.\\

Suppose we have a scaling function $\varphi$ such that
\begin{itemize}
    \item $\{\varphi(t - k): k \in \Zbb\}$ is orthonormal
    \item $\varphi$ satisfies the $2$-scale relation:
    \[\varphi(t) = \sum_{k \in \Zbb} p_k \varphi(2t - k)\]
    \item In particular, this implies the existence of a nested sequence of spaces
    \[\{0\} \subset ... \subset V_0 \subset V_1 \subset ... \subset... L^2(\Rbb)\]
\end{itemize}

\begin{question}
We have some questions:
\begin{itemize}
    \item Under what condition on $\varphi$ does it generate a multiresolution analysis?
    \item What is the associated wavelet $\psi$? $\psi$ is sometimes called the \textbf{mother wavelet}.
    \item What is $\psi$'s Two-Scale Relation?
    \item How to check if $\varphi$ is orthonormal?
\end{itemize}
We will see that many of these questions are dependent only on the choice of coefficients $p_k$.
\end{question}

We start with a simple condition for orthonormality.
\begin{theorem}
$\{\varphi(t - k): k \in \Zbb\}$ is orthonormal if and only if $\sum_{m \in \Zbb} |\widehat{\varphi}(w + 2m \pi)|^2 = 1$.
\end{theorem}

\begin{proof}
    By Parserval's Identity,
    \begin{align*}
    \int_{-\infty}^{\infty} \varphi(t - k) \overline{\varphi(t - \ell)} dt &= \frac{1}{2\pi} \int_{-\infty}^{\infty} e^{-ikw} \widehat{\varphi}(w) \overline{e^{-i\ell w} \widehat{\varphi}(w)} dw\\
    &= \frac{1}{2\pi} \int_{-\infty}^{\infty} e^{i(\ell -k)w} |\widehat{\varphi}(w)|^2 dw\\
    &= \frac{1}{2\pi} \sum_{m \in \Zbb} \int_{2m \pi}^{2(m+1)\pi} dw e^{i(\ell -k)w} |\widehat{\varphi}(w)|^2\\
    &= \frac{1}{2\pi} \sum_{m \in \Zbb} \int_{2m \pi}^{2(m+1)\pi} dw e^{i(\ell -k)w} |\widehat{\varphi}(w + 2m\pi)|^2 \tag*{U-substitute $w - 2m\pi \to w$}
    \end{align*}
Let $\Phi(w) = |\widehat{\varphi}(w + 2m\pi)|^2$, this is a $2\pi$-peridioc function, hence the integral here is really the Fourier Coefficient and
\begin{align*}
        \int_{-\infty}^{\infty} \varphi(t - k) \overline{\varphi(t - \ell)} dt = (\ell - k)-\text{th Fourier coefficient of }\Phi(w)
\end{align*}
We see that
\begin{align*}
    \Phi(w) = \sum_{n \in \Zbb} c_n e^{i n \omega}
\end{align*}
where $c_n = \frac{1}{2\pi} \int_{0}^{2\pi} dw e^{-inw} \Phi(w)$.\\

If we have orthonormality, then $c_0 = 1$. For $n \neq 0$, $c_n = 0$. Hence, $\Phi(\omega) = 1$ for all $\omega \in \Rbb$.\\

The converse also follows by tracting the steps backwards.
\end{proof}

\begin{example}
    For the Shannon Wavelet, $\varphi(t) = \frac{\sin \pi t}{\pi t}$ and $\widehat{\varphi}(w) = \begin{cases}
        1,\ |w| < \pi\\
        0, \text{ otherwise}
    \end{cases}$. We observe that the graphs of $\widehat{\varphi}(w + 2m\pi)$ are
    \[\includegraphics[width=0.6\textwidth]{Figures/lec28-1.png}\]
    Hence the sums add up to the constant $1$ function. Hence, the Shannon Wavelets are orthonormal.
\end{example}
...
\begin{proof}
    Let $f \in L^2(\Rbb)$ and $M \in \Rbb$ (to be determined) such that $M \to \infty$. We define the function
    \[f_M(t) = \begin{cases}
        f(t),\ |t| < M\\
        0, \text{ otherwise}
    \end{cases}\]
    So in particular $f_M \to f$ on $L^2(\Rbb)$ as $M \to \infty$. For $n \in \Zbb$, let's consider again the $L^2$ projection:
    \[L_n f_n = \sum_{k \in \Zbb} c_k^{(n)} \varphi(2^n t - k)\]
    In the case where that $\varphi$'s are orthonormal, we have that the coefficients are
    \[c_k^{(n)} = 2^n \int_\Rbb f_M(t) \overline{\varphi(2^n t - k)} dt = 2^n \int_{-M}^M f_M(t) \overline{\varphi(2^n t - k)} dt \quad (\dagger)\]
    What is the $L^2$ norm of $L_n f_M$? Well,
    \begin{align*}
        ||L_n f_M||^2 &= 2^{-n} \sum_{k \in \Zbb} ||c_k^{(n)}||^2
    \end{align*}
    To find the value of $ ||c_k^{(n)}||^2$, applying the Cauchy Schwartz Inequality to both sides of $(\dagger)$, we have that
    \begin{align*}
       2^{-n} ||c_k^{(n)}||^2 &\leq ||f_M||^2  2^{n} \int_{-M}^M dt |\varphi(2^n t - k)|^2\\
       &= ||f_M||^2  2^{n} \int_{-2^n M - k}^{2^n M-k} ds |\varphi(s)|^2 \tag*{Let $s = 2^n t - k$}
    \end{align*}
    Hence, we have that
    \begin{align*}
        ||L_n f_M||^2 &= 2^{-n} \sum_{k \in \Zbb} ||c_k^{(n)}||^2\\
        &\leq ||f_M||^2 \sum_{k \in \Zbb} \int_{-2^n M - k}^{2^n M-k} ds |\varphi(s)|^2
    \end{align*}
    What is the integral $\int_{-2^n M - k}^{2^n M-k} ds |\varphi(s)|^2$ we are integrating over here? Well, as $n \to -\infty$, the region we are integrating over would shrink to $0$ if $M$ is not growing fast enough:
    \[\includegraphics[width=0.5\textwidth]{Figures/lec29-1.png}\]
    Hence, as $n \to -\infty$, $||L_n f_M|| \to 0$. Now, we would also like $||L_n f||$ to go to $0$. Indeed, since $L_n$ is linear, by Triangle's Inequality, we have
    \[||L_n f|| \leq ||Ln f_M|| + ||L_n (f - f_M)|| \leq ||L_n f_M|| + ||f - f_M||\]
    where the second inequality comes from Pythagorean Theorem (projection reduces norm).\\
    ...
    But we have that $||f|| = 0$ if and only if $f = 0$. \mattie{almost everywhere $0$?}
\end{proof}
...
\end{question}
    ...
    We seek a solution of $(*)$ with compact support in $\supp(\varphi) = [A, B]$ for $A$ and $B$ to be determined.

    \begin{itemize}
        \item What is the support of $\varphi(2t)$? Well, $\varphi(2t) \neq 0$ if and only if $A \leq 2t \leq B$, which is true if and only if $t \in [\frac{A}{2}, \frac{B}{2}]$
        \item In general, the support of $\varphi(2t - j)$ is in $\frac{A + j}{2} \leq t \leq \frac{B+j}{2}$
    \end{itemize}

    Hence, using $(*)$, writing out the support on both sides
    \[[A, B] = \supp(\varphi) = \bigcup_{j=0}^3 [\frac{A+j}{2}, \frac{B+j}{2}] = [\frac{A}{2}, \frac{B+3}{2}]\]
    Hence we have that
    \[\frac{A}{2} = A \text{ and } \frac{B+3}{2} = B\]
    which implies that $A = 0$ and $B = 3$.\\

    Now we have that the graph $\varphi(t)$ should roughly look like (schematically)
    \[\includegraphics[width=0.6\textwidth]{Figures/lec31-1.png}\]
    Similarly, $\varphi(2t)$ is the same graph but shifted and scaled to $[0, \frac{3}{2}]$, $\varphi(2t - 1)$ is the same graph but shifted and scaled to $[\frac{1}{2}, 2]$, etc:
    \[\includegraphics[height=0.4\textheight]{Figures/lec31-2.png}\]

    Now, we want to figure out a procedure to get closer and closer to the solution. Let's suppose we have 
    $$\Vec{\varphi_n} = \text{values of $\varphi(t)$ at $t = k/2^n$ for $k = 0, 1, ..., 3 \times 2^n$}$$
    and let
    $$\Vec{\varphi_{n+1}} = \text{values of $\varphi(t)$ at $t = k/2^{n+1}$ for $k = 0, 1, ..., 3 \times 2^{n+1}$}$$
    The Two-Scale Relation allows us to compute $\Vec{\varphi_{n+1}}$ from $\Vec{\varphi_{n}}$.\\

    To code this, we can operate on vector directly:
    \[\includegraphics[width=0.6\textwidth]{Figures/lec31-3.png}\]
    To view this in a more tabular view:
    \[\includegraphics[width=0.4\textwidth]{Figures/lec31-4.png}\]
    Each time we compute $\Vec{\varphi}_{n+1}$ from $\Vec{\varphi}_{n}$, the points we get are twice as densely packed in the interval $[0, 3]$, and hence our solution gets finer and finer.\\

    What is the starting vector? Well, we want to choose the values of $\varphi(0) = 0, \varphi(1), \varphi(2), \varphi(3) = 0$. It turns out as long as we don't pick the zero vector to start with, our result would always converge to some interesting wavelet!

\begin{remark}
    How about the wavelet? How would be plot $\psi(t)$ Assuming the procedure above works, we just get the wavelet $\Vec{\psi}_{n+1}$ the same way but using the multipliers given by the two scale relation for $\psi(t)$.
\end{remark}


    Implementing the algorithm described above, we see the plots of the Daubechies 4 wavelet:
\[\includegraphics[height=0.5\textheight]{Figures/lec31-5.png}\]

\begin{question}
    How smooth is this wavelet? Is it even continuous? It's not so obvious now, but there's a whole branch of wavelets studying these properties. It turns out this example, the Daubechies 4 wavelet is only $C^{0.67}$ smooth (in terms of the Lipschitz Continuity).
\end{question}

\newpage
\section{Lecture 32 - 4/19/2023}

Given $\varphi$ satisfying
\begin{itemize}
    \item A Two-Scale Relation given by $\varphi(t) = \sum_j p_j \varphi(2t - j)$
    \item The family $\varphi(t - j)$ are orthornoaml.
\end{itemize}
We want to find an accompanying wavelet $\psi(t)$ with
\begin{itemize}
    \item $\psi(t) = \sum_j q_j \varphi(2t - j)$
    \item $\psi$ to also be an orthornoaml family.
\end{itemize}
...
The next step is not too intuitive but let's define $x = \sin^2 \frac{w}{2}$, so $\cos^2(w/2) = 1-x$. Let's define
\[F(x) = |R(w)|^2\]
So
\[|R(w+\pi)|^2 = F(\sin^2(w/2)) = F(1 - x)\]

Thus, $(\dagger)$ becomes
\[(1-x)^M F(x) + x^M F(1 - x) = 1, \forall x\]

\begin{remark}
    Thus, the question amounts down to choose a polynomial $F$ satsifying the condition
    \[(1-x)^M F(x) + x^M F(1 - x) = 1, \forall x\]
\end{remark}
...
\begin{example}
    When $M = 1$, we have
    \[(1-x) F(x) + x F(1 - x) = 1\]
    In this case we can just choose $F(x) = 1$, hence $|R(w)|^2 = 1$, so take $R(w) = 1$. Hence,
    \[P(w) = (\frac{1 + e^{-iw}}{2}) = \frac{1}{2}p_0 + \frac{1}{2} p_1 e^{-iw}\]
    So $p_0 = p_1 = 1$. This gives us back the Haar Wavelet!\\

    When $M = 2$, we have
    \[(1-x)^2 F(x) + x^2 F(1 - x) = 1\]
    We can choose $F(x) = 1 + 2x$ here, so
    \[|R(w)|^2 = 1 + 2 \sin^2(w/2) = 2 - \cos(w) = \frac{-z}{2} + 2 - \frac{\overline{z}}{2} \quad (\star)\]
    where $z = e^{-iw}$ and $\overline{z} = e^{iw} = 1/z$.\\

    By some magical choice, we can try $R(w) = \frac{z - r}{1 - r}$ so that $R(0) = 1$. Then
    \begin{align*}
        |R(w)|^2 &= \frac{z - r}{1 - r} \cdot \frac{\overline{z} - r}{1 - r}\\
        &= \frac{1 + r^2 - r(z + \overline{z})}{(1 - r)^2}
    \end{align*}
    Comparing this with $(\star)$ tells that $r$ has to satisfy
    \[-r + 4 - \frac{1}{r} = 0\]
    Hence we have that
    \[R(w) = \frac{e^{-iw} - r}{1 - r}\]
    So we have that
    \begin{align*}
        P(w) &= 
    \end{align*}
    We get Daubechies's $D-4$ Wavelet.
\end{example}
